...
def review_top_findings(output_dir, findings, info, ai_review_fn):
    ...
    for f in sorted_f:
        ...
        if fp and fp not in reviewed and Path(fp).exists():
            reviewed.add(fp)
            to_review.append(fp)
        if len(to_review) >= MAX_FILES: break
    if not to_review:
        print("  ⚠️  No files for AI review")
        return findings
    print(f"  📂 Reviewing {len(to_review)} files...")
    ai_notes = []
    for i, file_path in enumerate(to_review, 1):
        ...
        flags = "\n".join(f"- [{f['severity']}] {f['name']}: {f.get('content','')[:80]}"
...
Mark scanner false positives. Be concise."""
...
def _parse_ai(text, file_path, findings):
    for m in re.finditer(r'(?i)\b(CRITICAL|HIGH|MEDIUM|LOW)\b[:\s]+([^\n]{10,80})', text):
        ...
        if sev in ("CRITICAL","HIGH","MEDIUM"):
            findings.append({"module":"ai_review","name":f"AI: {desc[:60]}",
                              "severity":sev,"file":file_path,"line":"","content":desc})
