...
SSL_PATTERNS = [
    ...
    (r'NSAllowsArbitraryLoads.*true', "ATS disabled (iOS)", "HIGH"),
]
...
CRYPTO_PATTERNS = [
    ...
    (r'new\s+Random\s*\(\s*\)', "Insecure Random (use SecureRandom)", "MEDIUM"),
...
WEBVIEW_PATTERNS = [
    ...
    (r'setAllowUniversalAccessFromFileURLs\s*\(\s*true\s*\)', "Universal file access in WebView", "CRITICAL"),
]

STORAGE_PATTERNS = [
    (r'MODE_WORLD_READABLE|MODE_WORLD_WRITEABLE', "World-readable/writable storage", "HIGH"),
...
SQLI_PATTERNS = [
    (r'rawQuery\s*\([^,)]*\+', "SQL injection risk in rawQuery", "HIGH"),
    (r'execSQL\s*\([^,)]*\+', "SQL injection risk in execSQL", "HIGH"),
]
...
def hunt_app(output_dir: str, info: dict) -> List[Dict]:
    ...
    if not scan_dirs:
        print("  ⚠️  No source directories to scan")
        return []
...
def _run_scan(module, patterns, scan_dirs, findings_dir):
    ...
    if findings:
        ...
        print(f"  [{module.upper():10s}] {len(findings)} issues found")
    else:
...
def _filter_fp(findings):
    fp_pats = [
        r'/test/', r'Test\.java', r'BuildConfig\.java',
        r'/R\.java', r'example\.com', r'YOUR_KEY',
        r'placeholder', r'xxx+'
    ]
    return [
        f for f in findings
