...
def _unpack_apk(apk_path, out_dir):
    ...
    # jadx
    if not any(decompiled.rglob("*.java")) if decompiled.exists() else True:
        print("  🔧 Running jadx...")
        if not _tool_exists("jadx"):
            print("  ❌ jadx not found. Install: https://github.com/skylot/jadx/releases")
            sys.exit(1)
        ret = subprocess.run(
        ...
        print(f"  ✅ jadx: {len(java_files)} Java files")
    else:
        print(f"  ✅ jadx: already done")
...
def _unpack_ipa(ipa_path, out_dir):
    ...
    if any(raw.rglob("*.jsbundle")): fw = "React Native"
    elif any(raw.rglob("flutter_assets")): fw = "Flutter"
    (out_dir / "framework.txt").write_text(fw)
    print(f"  📱 Framework: {fw}")
    return str(out_dir)
...
def _detect_framework(decompiled, raw):
    if any(raw.rglob("libreactnativejni.so")) or any(raw.rglob("index.android.bundle")):
        return "React Native"
    if any(raw.rglob("libflutter.so")): return "Flutter"
    if any(raw.rglob("Mono.Android.dll")): return "Xamarin"
    if any(raw.rglob("cordova.js")): return "Cordova/Ionic"
